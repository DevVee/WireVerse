...
import { camera, checkCol } from './world.js';
import { Audio } from './audio.js';

// ── PLAYER STATE ─────────────────────────────────────────────────────────────
export const Player = {
  pos: new THREE.Vector3(2, 1.72, 10),
  yaw: Math.PI, pitch: 0,
  yawVel: 0, pitchVel: 0,
  vel: new THREE.Vector3(),
  bobT: 0, bobAmt: 0,
  BASE_Y: 1.72,
  SPEED: 5.2, SPRINT: 9.5,
  sprinting: false,
};
...
lookZone.addEventListener('touchmove', e => {
  e.preventDefault();
  for (const t of e.changedTouches) if (lookTouches[t.identifier]) {
    Player.yawVel   -= (t.clientX - lookTouches[t.identifier].x) * 0.0024;
    Player.pitchVel -= (t.clientY - lookTouches[t.identifier].y) * 0.0024;
...
export function updatePlayer(dt) {
  ...
  np.x = Math.max(-14.5, Math.min(24.5, np.x));
  np.z = Math.max(-21.5, Math.min(23.5, np.z));
  if (!checkCol(np)) {
    Player.pos.copy(np);
  } else {
  ...
  Player.pos.y = Player.BASE_Y + curSin * 0.03 * Player.bobAmt;
