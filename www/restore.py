...
html_path = pathlib.Path("A:/Electric/www/index.html")
html = html_path.read_text(encoding="utf-8")

replacement = """    .register-input:focus { border-color: #ffd700; }
    .register-input::placeholder { color:rgba(255,255,255,.25); }
    .register-error {
...
# split at .register-input:focus { border-color: #ffd700; }
a = "    .register-input:focus { border-color: #ffd700; }"
blocks = html.split(a)
assert len(blocks) == 2

b = "    <!-- Electrical Room Background Decorations -->"
blocks2 = blocks[1].split(b)
assert len(blocks2) >= 2

new_html = blocks[0] + replacement + b + "".join(blocks2[1:])
